...
- 从 v0.3.4 开始，独立的 Windows 计划任务会在 Codex 重新启动或常驻进程退出后一分钟内恢复插件，即使 Codex Hook 没有运行也能恢复。
...
这会创建当前用户专用的隐藏计划任务 `\CodexUsageCompanion\Recovery`、立即启动，并每分钟检查一次。无需管理员权限，也不会留下常驻 PowerShell 进程；原有 Hook 仍负责即时启动和更新。
